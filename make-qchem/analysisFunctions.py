...
#            lambda_i[idx, :] = tset["lambda"][t0idx, tset["t0"][t0idx]-trev:(tfor+tset["t0"][t0idx]+1)]
        ...
        #trajid = [0 for idx in range(revdcd.shape[0])] + [1 for idx in range(fordcd.shape[0])]
    #if rev_r[-1] > P_lower:
    #    fulltraj[:fordcd.shape[0], :, :] = fordcd[::-1, :, :]
    #    fulltraj[fordcd.shape[0]:, :, :] = revdcd
    #    trajid_isrev = 1 #[1 for idx in range(fordcd.shape[0])] + [0 for idx in range(revdcd.shape[0])]
...
# The following do not check how far you've made progress loading trajs
#def choose_trajectories(qmregion, Ntrajs, trajloc, Nframes, rseed, fxnkey, R_upper, rxncoor_atoms):
#    """
#    The following script will, given Ntrajs, pick (from each cluster) this amount of trajectories randomly
#    to be able to calculate the reactive/nonreactive performance.
#
...
#    #idx - counter over trajectories, traj_idx - the actual number you wish to cull from
#    for idx, traj_idx in enumerate(trajs):
#        fulltraj, irev = readdcd_full(trajloc[traj_idx], qmregion, rxncoor_atoms, fxnkey)
#        trajcoor[idx, :, :, :] = fulltraj[:Nframes, :, :]
#        isrev.append(irev)
#        t0.append(get_troughs_oscillatory(trajcoor[idx, :, :, :], rxncoor_atoms, fxnkey, R_upper))
#        rxncoor[idx, :] = get_rxncoor(trajcoor[idx, :, :, :], rxncoor_atoms = rxncoor_atoms, fxnkey = fxnkey)
#    return trajcoor, [trajloc[idx] for idx in trajs], t0, isrev, rxncoor


#def load_trajectories(clustid, trajloc, qmregion, Ntrajs, Nframes, rseed, rxncoor_atoms, fxnkey, R_upper):
#        """
#        After assigning the cluster map, load the trajectories
#
#        Input:
...
#        trajs = {}
#        trajset = trajloc[clustid]
#        trajcoor, trajnames, t0, isrev, rxncoor = choose_trajectories(qmregion, Ntrajs, trajset, Nframes = Nframes, rseed = rseed, fxnkey = fxnkey, R_upper = R_upper, rxncoor_atoms = rxncoor_atoms)
#        trajs.update({clustid : {"Trajs" : trajcoor, "Trajnames" : trajnames, "t0" : t0, "isrev" : isrev, "lambda" : rxncoor}})
#        print("Finished collecting trajectories for cluster = " + str(clustid) )
#        return trajs
...
#The following two functions are split because
#Reactives continue on from the reactive basin but nonreactives return.

#def get_troughs_oscillatory_reac(trajdcd, rxncoor_atoms, fxnkey, R_upper):
#    """
#    Given a trajectory, it will compute the last trough between the two atoms of the reaction coordinate.
#
#    The t0 is defined by oscillatory bonds. You can also ask the last time r < R_upper (or last time in Reactant Basin)
...
#    """
#    r = get_rxncoor(trajdcd, rxncoor_atoms, fxnkey)
#    tp = [t for t in range(len(r)) if r[t] <= R_upper]
#    rtp = [r[idx] for idx in tp]
#    t0 = [t for t in range(1, len(rtp) - 1) if rtp[t] <  rtp[t+1] and rtp[t] < rtp[t-1]] #explicitly the distance is less than the step before and after it.
#    if len(t0):
#        t0 = t0[-1] #Find the last time you were in the reactant basin trough
#    else:
#        t0 = 0
...
#def get_troughs(trajdcd, rxncoor_atoms, fxnkey = 'dist', R_upper = 1.8):
#    """
#    Given a trajectory, it will compute the last trough between the two atoms of the reaction coordinate.
#
#    The t0 is defined by oscillatory bonds. You can also ask the last time r < R_upper (or last time in Reactant Basin)
...
#
#    """
#    r = fxntype[fxnkey]([trajdcd[:, idx, :] for idx in rxncoor_atoms])
#    tp = [t for t in range(1, len(r)) if r[t] >= R_upper] #Find the times you escape the reactant basin
#    if len(tp):
#        t0 = tp[-1]-1 #Find the last time you escape it
#    else:
#        t0 = 0
#    return t0

#def choose_trajectories(qmregion, Ntrajs, trajloc, Nframes, rseed, fxnkey, R_upper, P_lower):
#    """
#    The following script will, given Ntrajs, pick (from each cluster) this amount of trajectories randomly
#    to be able to calculate the reactive/nonreactive performance.
#
...
#    #idx - counter over trajectories, traj_idx - the actual number you wish to cull from
#    for idx, traj_idx in enumerate(trajs):
#        trajcoor[idx, :, :, :] = readdcd(trajloc[traj_idx], qmregion)[:Nframes, :, :]
#    return trajcoor, [trajloc[idx] for idx in trajs]


#def load_trajectories(Nclust, trajloc, qmregion, Ntrajs, Nframes, rseed):
#        """
#        After assigning the cluster map, load the trajectories
#
#        Input:
#        -----
#        Nclust    - clusters you want to collect trajectories from
#        trajloc   - mapping of the trajectory for each cluster
#        qmregion  - quantum region; index of atoms
#        Ntrajs    - Number of trajectories you want to pull from each cluster
#        Nframes   - Number of frames in the dataset
...
#        """
#        trajs = {}
#        for clustid in Nclust:
#            trajset = trajloc[clustid]
#            trajcoor, trajnames = choose_trajectories(qmregion, Ntrajs, trajset, Nframes = Nframes, rseed = rseed)
#            trajs.update({clustid : {"Trajs" : trajcoor, "Trajnames" : trajnames}})
#            print("Finished collecting trajectories for cluster = " + str(clustid) )
#        return trajs



#def choose_trajectories_criteria(qmregion, Ntrajs, trajloc, Nframes = 1, criteria = {'t' : [], 'lambda_0' = []}, rseed = 1234):
#    """
#    The following script will, given Ntrajs, pick (from each cluster) this amount of trajectories randomly
#    to be able to calculate the reactive/nonreactive performance.
#
...
#    trajloc  - mapping of the trajectories you wish to process
#    Nframes  - number of frames in the trajectory you wish to sample
#    criteria - a dictionary that indicates the upper/lower bounds of reaction coordinates, and troughs.
#               "t" - time coordinate; value is [t_lower, t_upper]. If you do not care for this criteria, leave it as "False", "False"
#               "lambda_i" - rxn coordinate i, format is as follows:
#                        [[atomind1, atomind2], l_lower, l_upper]
#               You want to ensure the following criteria are met:
#               1) There is at least t_lower fs backward and t_upper fs forward
#               2) The reaction coordinate fulfills some criteria
#    rseed    - Random seed used to generate the random numbers to sample from
#
#    Outputs:
#    -------
...
#
#    """
#    if 't' not in criteria.keys():
#        print("You did not specify time criteria using 't'. Ignoring it.")
#        criteria.update({'t' : []})
#    t_criteria = criteria['t']
#    rxncoor = [key for key in criteria.keys() if 'lambda_' in key]
#    trajs = np.zeros(shape = (Ntrajs, Nframes, len(qmregion), 3))
#    random.seed(rseed)
#    Nseeds = random.shuffle(range(Ntrajs))
